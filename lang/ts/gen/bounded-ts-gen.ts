...
export class BoundedTsGenerator {
    ...
    private emitProperty(type: BI.BoundedType): TS.TsProperty {
        ...
        switch (type.kind) {
            ...
            case 'ref':
                {
                    const ref = this.bounded.getType(type)
                    result = {
                        kind: 'property',
                        name: this.emitTsVarName(type),
                        type: {
    ...
    private emitImports(type: BI.BoundedObject): TS.TsImport[] {
        ...
        const process = (prop: BI.BoundedType) => {
            switch (prop.kind) {
                case 'array':
                    process(prop.items)
                    break
                case 'ref':
                    {
                        const ref = this.bounded.getType(prop)
    ...
    private emitValidation(sb: StringBuffer, type: BI.BoundedType, options?: Partial<EmitValidationOptions>) {
    ...
    private emitSql(name: string, fn: (sb: StringBuffer) => void): TS.TsMethodDecl {
        const sb = new StringBuffer()
        sb.l('return `')
        sb.nl()
        sb.i(() => { fn(sb) }, 4)
    ...
    private emitSqlCreate(type: BI.BoundedObject): TS.TsMethodDecl {
        return this.emitSql('sqlCreate', (sb: StringBuffer) => this.sqlGen.emitCreate(sb, type))
    }

    private emitSqlInsert(type: BI.BoundedObject): TS.TsMethodDecl {
        return this.emitSql('sqlInsert', (sb: StringBuffer) => this.sqlGen.emitInsert(sb, type))
    }

    private emitSqlSelect(type: BI.BoundedObject): TS.TsMethodDecl {
        return this.emitSql('sqlSelect', (sb: StringBuffer) => this.sqlGen.emitSelect(sb, type))
    }

    private emitSqlUpdate(type: BI.BoundedObject): TS.TsMethodDecl {
        return this.emitSql('sqlUpdate', (sb: StringBuffer) => this.sqlGen.emitUpdate(sb, type))
    }
